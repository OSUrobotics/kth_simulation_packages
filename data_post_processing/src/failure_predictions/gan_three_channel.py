...
class GAN:
    ...
    def generator(self):
        ...
        self.G.add(Conv2D(depth,kernel_size=self.kernel_size,strides=2,input_shape=(self.img_rows,self.img_cols,2),data_format='channels_last',padding='same'))
    ...
    def generator_model(self):
        ...
        paths = Input(shape = (self.img_rows,self.img_cols,1))
    ...
    def discriminator_model(self):
        ...
        input = concatenate([img,paths,heatmap])
        output = self.discriminator()(input)
        self.DM = Model([img,paths,heatmap],output)
    ...
    def adversarial_model(self):
        ...
        img = Input(shape=(self.img_rows,self.img_cols,1))
        paths = Input(shape=(self.img_rows,self.img_cols,1))
        truth = Input(shape=(self.img_rows,self.img_cols,1))
        generator_model = self.generator_model()
        discriminator_model = self.discriminator_model()

        # heatmap = generator_model([img,noise])
        heatmap = generator_model([img,paths])
        # make discriminator not train when training generator
        discriminator_model.trainable = False
        output = discriminator_model([img,paths,heatmap])
        # self.AM = Model([img,noise],output)
        self.AM = Model([img,paths,truth],[output,heatmap])
        self.AM.compile(loss=['binary_crossentropy','mse'], loss_weights=[1, 100], optimizer=optimizer)
        return self.AM
    ...
    def train(self, data_path, pattern, epochs,batch_size=128, sample_interval=100, output_path = None):
        ...
        for epoch in range(epochs):
            ...
            d_loss_real = discriminator_model.train_on_batch([imgs,paths,heatmaps],valid)
            d_loss_fake = discriminator_model.train_on_batch([imgs,paths,gen_heatmaps],fake)
...
if __name__ == "__main__":
    ...
    g = GAN()
    validation_files = g.train(sys.argv[1],'path_failure_training_data.npz', epochs=20000, batch_size = 32, sample_interval = 500, output_path = output_folder)
