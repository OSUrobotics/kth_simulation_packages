...
class GAN:
    ...
    def generator(self):
        ...
        self.G.add(Conv2D(depth,kernel_size=self.kernel_size,strides=2,input_shape=(1024,1024,1),data_format='channels_last',padding='same'))
    ...
    def discriminator_model(self):
        ...
        input = concatenate([paths,img])
        output = self.discriminator()(input)
        self.DM = Model([paths,img],output)
    ...
    def adversarial_model(self):
        ...
        gen_paths = generator_model(imgs)
        # make discriminator not train when training generator
        discriminator_model.trainable = False
        output = discriminator_model([paths,imgs])
        # self.AM = Model([img,noise],output)
        self.AM = Model([imgs,paths],[output,gen_paths])
    ...
    def train(self, data_path, pattern, epochs,batch_size=32, sample_interval=100, output_path = None):
        ...
        for epoch in range(epochs):
            ...
            imgs, paths = X_maps[idx], X_paths[idx]
            # noise = np.random.normal(0,1, (batch_size,100))

            # gen_heatmaps = generator_model.predict([imgs,noise])
            gen_paths = generator_model.predict(imgs)
            ...
            d_loss_real = discriminator_model.train_on_batch([paths,imgs],valid)
            d_loss_fake = discriminator_model.train_on_batch([gen_paths,imgs],fake)
            d_loss = 0.5 * np.add(d_loss_real, d_loss_fake)
            # g_loss = adversarial_model.train_on_batch([sampled_ims,noise],valid)
            discriminator_model.trainable = False
            g_loss = adversarial_model.train_on_batch([imgs,paths],[valid,paths])
            ...
            if epoch % sample_interval == 0:
                self.sample_images(epoch,X_maps_original,X_paths_original,output_path)
        return validation_files


    def sample_images(self,epoch,X_maps, X_paths,output_path):
        if not output_path:
            return 0
        r,c = 1,3
        ...
        axs[2].imshow(sampled_paths[cnt,:,:,0],cmap='plasma')
...
if __name__ == "__main__":
    ...
    output_folder = sys.argv[2] + date + '_full_map_highway_prediction/'
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    g = GAN()
    validation_files = g.train(sys.argv[1],'full_map_highway_training_data.npz', epochs=50000, batch_size=8, sample_interval = 500, output_path = output_folder)
