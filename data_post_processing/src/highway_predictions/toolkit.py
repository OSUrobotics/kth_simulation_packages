...
def plot_points(data,img):
    ...
    for point in data['recovery_locations']:
        ...
        # img[y,x] = (0,0,255)
        if point["recovery_behavior"] == "rotate_recovery":
            cv2.circle(img,(x,y),2,color=(0,0,255),thickness = -1)
        else:
            cv2.circle(img,(x,y),2,color=(255,0,0),thickness = -1)
...
def generate_failure_rate_heatmap(raw_data_folder, resolution_scaling=1.0):
    files = glob.glob(raw_data_folder+ '*.npy')
    im = imread(raw_data_folder + 'map.pgm')
    config = yaml.load(open(raw_data_folder+'map.yaml','rb'))
    res = config['resolution']
    ...
    y_org = -origin[1]
    height,width = im.shape
    scaled_heatmap = np.zeros((int(height/resolution_scaling),int(width/resolution_scaling),2))
    h,w,c = scaled_heatmap.shape
    pose_tuples = load_failure_locations(files)
    for pose in pose_tuples:
        x = int((pose[0] + x_org) / scaled_res)
        y = h - int((pose[1] + y_org) / scaled_res)
        scaled_heatmap[y,x,1] += 1.0
    return_im = np.zeros((height,width))
    for i in range(height):
        for j in range(width):
            if scaled_heatmap[int(i/resolution_scaling),int(j/resolution_scaling),1] > 1:
                return_im[i,j] = scaled_heatmap[int(i/resolution_scaling),int(j/resolution_scaling),1]

    # find reachable space
    reachable = get_samples_wavefront(im, origin, res, sample_rate = 1.0, max_samples = float('inf'), rad=0.0)
    vals = dict()
    for i in reachable:
        vals[i] = return_im[i[0],i[1]]
        ...
        if (k % 2) == 0:
            for i,j in reachable:
                ...
                for y,x in [(i-1,j),(i+1,j),(i,j-1),(i,j+1)]: #,(i-1,j-1),(i+1,j-1),(i-1,j+1),(i+1,j+1)
                    try:
                        qs.append(.99*vals[y,x])
                    except:
                        pass
                vals[i,j] = max(qs)
        else:
            for i,j in reversed(reachable):
                # value iteration
                qs = [vals[i,j]]
                for y,x in [(i-1,j),(i+1,j),(i,j-1),(i,j+1)]: #,(i-1,j-1),(i+1,j-1),(i-1,j+1),(i+1,j+1)
                    try:
    ...
    for key,value in vals.iteritems():
        return_im[key[0],key[1]] = value
    return_im = return_im / return_im.max()
    return return_im

def generate_failure_rate_training_data(filepath,samples=500,window_size = 12.8):
    seed = int(time.time())
    np.random.seed(seed)
    try:
...
def generate_highway_training_data(filepath, samples = 500, window_size = 12.8):
    seed = int(time.time())
    np.random.seed(seed)
    try:
        highway_values = np.load(filepath + 'visits.npy')
    except:
        print('no visits.npy file found in ' + filepath)
        sys.exit(1)

    height,width = highway_values.shape
    config = yaml.load(open(filepath+ 'map.yaml','rb'))
    res = config['resolution']
    im = imread(filepath + 'map.pgm')
    ...
    while i < samples:
        ...
        if x_map.mean() == x_map.max():
            ...
            x_map = x_map.reshape(x_map.shape[0],x_map.shape[1],1)
            x_heatmap = _get_window(highway_values,y,x,window_size,res)
            if x_heatmap.max() < 1:
                pass
            else:
                mx = float(x_heatmap.max())
                x_heatmap = x_heatmap / mx
                x_heatmap = x_heatmap.reshape(x_heatmap.shape[0],x_heatmap.shape[1],1)
                point = np.concatenate([x_map,x_heatmap],axis=-1)
                data.append(point)
                i += 1
